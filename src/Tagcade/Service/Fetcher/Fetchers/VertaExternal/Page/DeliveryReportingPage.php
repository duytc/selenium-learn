...
use Tagcade\Service\Fetcher\Fetchers\VertaExternal\Widget\DateSelectWidget;
...
class DeliveryReportingPage extends AbstractPage
{
    ...
    private function clickReportOrBarGraphSymbol()
    {
        try {
            ...
            $reportsBtn = $this->driver->findElement(WebDriverBy::id('ext-element-14'));
            $reportsBtn->click();
        } catch (\Exception $e) {
            $this->clickReportOrBarGraphSymbol();
        }
    ...
    private function selectReportType($reportType)
    {
        try {
            ...
            $reportTypeId = VertaExternalUtil::getId(VertaExternalUtil::RADIO, $reportTypeId);
    ...
    {
        try {
            $sliceId = VertaExternalUtil::getId(VertaExternalUtil::BUTTON, $this->specialId + 30);
            $sliceComboBox = $this->driver->findElement(WebDriverBy::id($sliceId));
            $sliceComboBox->click();

            $id = sprintf('menu-%s-innerCt', $this->specialId + 31);
    ...
    {
        try {
            $exportTabId = VertaExternalUtil::getId(VertaExternalUtil::BUTTON, $this->specialId - 11);
            $exportTab = $this->driver->findElement(WebDriverBy::id($exportTabId));
            $exportTab->click();

            $exportToCSVId = VertaExternalUtil::getId(VertaExternalUtil::MENU_ITEM, $this->specialId - 9);
            $exportToCSVButton = $this->driver->findElement(WebDriverBy::id($exportToCSVId));
